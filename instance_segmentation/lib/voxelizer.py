...
class Voxelizer:
  ...
  def voxelize(self, coords, feats, labels, instance_labels, center=None):
    ...
    if self.clip_bound is not None:
      ...
      if clip_inds is not None:
        ...
        if instance_labels is not None:
          instance_labels = instance_labels[clip_inds]
    ...
    # coords_au, feats, labels, map = ME.utils.sparse_quantize(
    #     coords_aug, feats, labels=labels, ignore_label=self.ignore_label, return_index=True)
    # coords_aug, feats, labels, map = ME.utils.sparse_quantize(
    #     coords_aug, feats, labels=labels, return_index=True)
    # mapping, colabels = ME.utils.sparse_quantize(
    #     coords_aug, features=feats, labels=labels, return_index=True, ignore_label=self.ignore_label)
    coords_aug1, map = ME.utils.sparse_quantize(
        coords_aug, return_index=True)
    ...
    feat = feats[map]
    label = labels[map]
    instances = instance_labels[map]
    # print(map.shape)
    # print(feat.shape)
    # print(label.shape)
    # print(instances.shape)

    return coords_aug1.numpy(), feat, label, instances, map
