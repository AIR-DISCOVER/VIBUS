...
class VoxelizationDatasetBase(DictDataset, ABC):
  ...
  def load_ply(self, index):
    ...
    plydata = torch.load(filepath)
    coords = plydata[0]
    feats = plydata[1]
    labels = plydata[2]
    instance_labels = plydata[3]
    # data = plydata.elements[0].data
    # coords = np.array([data['x'], data['y'], data['z']], dtype=np.float32).T
    # feats = np.array([data['red'], data['green'], data['blue']], dtype=np.float32).T
    # labels = np.array(data['label'], dtype=np.int32)
    # instance_labels = np.array(data['instance_label'], dtype=np.int32)
    return coords, feats, labels, instance_labels
...
class VoxelizationDataset(VoxelizationDatasetBase):
  ...
  def get_instance_info(self, xyz, instance_ids):
    ...
    '''
    centers = np.ones((xyz.shape[0], 3), dtype=np.float64) * -1   # (n, 9), float, (cx, cy, cz, minx, miny, minz, maxx, maxy, maxz, occ, num_instances)
    occupancy = {}   # (nInst), int
    bbox = {}
  ...
  def __getitem__(self, index):
    ...
    coords, feats, labels, instances = self.load_ply(index)
    ...
    coords1, feats1, labels1, instance1, transformation = self.voxelizer.voxelize(
    ...
    #   coords2, feats2, labels2 = self.input_transform(coords1, feats1, labels1)
    # if self.target_transform is not None:
    #   coords2, feats2, labels2 = self.target_transform(coords1, feats1, labels1)

      #----------------Instances-------------------------
    instance_info = instance1
    condition = (labels1 == self.ignore_mask)
    # print(condition.shape)
    # print(instance1.shape)
    instance1[condition] = -1
    IGNORE_LABELS_INSTANCE = self.IGNORE_LABELS if self.config.train_stuff else self.IGNORE_LABELS_INSTANCE
    for ignore_id in IGNORE_LABELS_INSTANCE:
        condition = (labels1 == ignore_id)
        instance1[condition] = -1
    instance_info = self.get_instance_info(coords1, instance1)


    if self.IGNORE_LABELS is not None:
      labels2 = np.array([self.label_map[x] for x in labels1], dtype=np.int)

    # Use coordinate features if config is set
    # if self.AUGMENT_COORDS_TO_FEATS:
    #   coords4, feats4, labels4 = self._augment_coords_to_feats(coords2, feats2, labels3)

    # return_args = [coords1, feats1, labels2, instance_info]
    return_args = [coords1, feats1, labels2, instance_info, transformation]    #is_view
