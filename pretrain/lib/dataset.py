...
class VoxelizationDataset(VoxelizationDatasetBase):
  ...
  def __getitem__(self, index):
    ...
    if self.PREVOXELIZATION_VOXEL_SIZE is not None:
      inds = ME.utils.sparse_quantize(
          coords / self.voxel_size, return_index=True)
      coords = coords[inds]
      feats = feats[inds]
      labels = labels[inds]
        # Prevoxel transformations
    ...
    coords1, feats1, labels1, coords2, feats2, labels2 = self.voxelizer.voxelize(
        coords1, coords2, feats1, labels1, feats2, labels2, center=center)
