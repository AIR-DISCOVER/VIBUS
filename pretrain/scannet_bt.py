...
SCANNET_OUT_PATH = Path('/home/aidrive1/workspace/luoly/dataset/Min_scan/bt_train/')
TRAIN_DEST = 'train'
TEST_DEST = 'test'
...
def handle_process(path):
  ...
  processed = pointcloud[:, :6]
  save_point_cloud(processed, out_f, with_label=False, verbose=True)
...
for files, bug_index in BUGS.items():
  ...
  for f in SCANNET_OUT_PATH.glob(files):
    ...
    save_point_cloud(pointcloud, f, with_label=False, verbose=False)
